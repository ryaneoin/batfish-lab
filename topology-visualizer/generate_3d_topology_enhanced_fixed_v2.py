...
POST_SCRIPT = r'''(function () {
  function boot() {
    const gd = document.getElementById('{plot_id}');
    if (!gd) return requestAnimationFrame(boot);

    // Make Safari console less mysterious
    window.addEventListener('error', (e) => {
      try { console.error('[layer-toggle] window.error', e.message, e.error); } catch (_) {}
    });

    if (!gd.data || !gd.data.length) return requestAnimationFrame(boot);

    const nodeIdx = gd.data.findIndex(t => t && t.name === 'Devices' && t.type === 'scatter3d');
    if (nodeIdx === -1) {
      console.warn('[layer-toggle] Could not find Devices trace; traces:', gd.data.map(t => t && t.name));
      return;
    }

    const nodeT = gd.data[nodeIdx];

    if (!Array.isArray(nodeT.customdata) || nodeT.customdata.length === 0) {
      console.warn('[layer-toggle] Devices trace has no customdata (layer keys).');
      return;
    }

    // Preserve originals once (so toggling is reversible)
    if (!nodeT.__orig) {
      nodeT.__orig = {
        x: (nodeT.x || []).slice(),
        y: (nodeT.y || []).slice(),
        z: (nodeT.z || []).slice(),
        text: (nodeT.text || []).slice(),
        hovertext: (nodeT.hovertext || []).slice(),
        customdata: (nodeT.customdata || []).slice(),
        mode: nodeT.mode
      };
    }

    const origNodes = nodeT.__orig;
    ...
    // Map node coordinate -> layer key
    const coordToLayer = new Map();
    for (let i = 0; i < origNodes.x.length; i++) {
      coordToLayer.set(keyOf(origNodes.x[i], origNodes.y[i], origNodes.z[i]), String(origNodes.customdata[i]));
    }

    const uniqueLayers = [];
    const seen = new Set();
    for (const l of origNodes.customdata) {
      const k = String(l);
      if (!seen.has(k)) { seen.add(k); uniqueLayers.push(k); }
    }

    // Collect all edge-like traces (lines)
    const lineTraceIdxs = [];
    for (let i = 0; i < gd.data.length; i++) {
      const t = gd.data[i];
      if (!t) continue;
      if (t.type !== 'scatter3d') continue;
      if (!t.mode || !String(t.mode).includes('lines')) continue;

      // Preserve originals for each edge trace
      if (!t.__orig) {
        t.__orig = { x: (t.x || []).slice(), y: (t.y || []).slice(), z: (t.z || []).slice() };
      }
      lineTraceIdxs.push(i);
    }

    const hidden = new Set();

    // Panel (top-right)
    const old = document.getElementById('z-layer-controls');
    if (old) old.remove();

    const panel = document.createElement('div');
    panel.id = 'z-layer-controls';
    ...
    title.textContent = 'Z Layers';
    title.style.fontWeight = '700';
    title.style.marginBottom = '8px';
    panel.appendChild(title);

    function applyFilter() {
      // Nodes: filter out points whose layer is hidden
      const nx = [], ny = [], nz = [], ntext = [], nhover = [], ncd = [];
      for (let i = 0; i < origNodes.x.length; i++) {
        const layer = String(origNodes.customdata[i]);
        if (!hidden.has(layer)) {
          nx.push(origNodes.x[i]);
          ny.push(origNodes.y[i]);
          nz.push(origNodes.z[i]);
          if (origNodes.text && origNodes.text.length) ntext.push(origNodes.text[i]);
          if (origNodes.hovertext && origNodes.hovertext.length) nhover.push(origNodes.hovertext[i]);
          ncd.push(origNodes.customdata[i]);
        }
      }

      nodeT.x = nx; nodeT.y = ny; nodeT.z = nz;
      if (origNodes.text && origNodes.text.length) nodeT.text = ntext;
      if (origNodes.hovertext && origNodes.hovertext.length) nodeT.hovertext = nhover;
      nodeT.customdata = ncd;
      nodeT.mode = origNodes.mode;

      // Edges: for each line trace, keep only segments whose endpoints are visible
      for (const idx of lineTraceIdxs) {
        const t = gd.data[idx];
        const ox = t.__orig.x, oy = t.__orig.y, oz = t.__orig.z;
        const ex = [], ey = [], ez = [];

        const pushBreak = () => { ex.push(null); ey.push(null); ez.push(null); };

        // Segments are typically [x0,x1,null,x2,x3,null,...]
        let seg = [];
        for (let i = 0; i <= ox.length; i++) {
          const isBreak = (i === ox.length) || (ox[i] == null || oy[i] == null || oz[i] == null);
          if (!isBreak) {
            seg.push(i);
          } else {
            if (seg.length >= 2) {
              const iA = seg[0], iB = seg[1];
              const layerA = coordToLayer.get(keyOf(ox[iA], oy[iA], oz[iA]));
              const layerB = coordToLayer.get(keyOf(ox[iB], oy[iB], oz[iB]));

              // Keep only if both endpoints map to visible layers.
              // If we cannot map an endpoint to a layer, keep the segment (safer than losing data).
              const aHidden = (layerA != null) ? hidden.has(String(layerA)) : false;
              const bHidden = (layerB != null) ? hidden.has(String(layerB)) : false;
              const keep = !(aHidden || bHidden);

              if (keep) {
                for (const j of seg) {
                  ex.push(ox[j]); ey.push(oy[j]); ez.push(oz[j]);
                }
                pushBreak();
              }
            }
      ...
      console.log('[layer-toggle] hidden layers:', Array.from(hidden));
    }
    ...
    for (const layer of uniqueLayers) {
      ...
      cb.addEventListener('change', () => {
        if (cb.checked) hidden.delete(layer);
    ...
    note.textContent = 'Toggles hide nodes + edges for each layer.';
    ...
    console.log('[layer-toggle] ready. layers=', uniqueLayers, 'lineTraces=', lineTraceIdxs.map(i => gd.data[i] && gd.data[i].name));
    applyFilter();
  }
...
def visualize_3d_topology_enhanced(graphs, datacenter=None):
    ...
    node_layer_keys = []  # per-node layer label for JS toggles
    node_text = []
    node_symbols = []
    node_sizes = []
    node_colors = []
    ...
    for node in pos_3d.keys():
        ...
        # Track which Z-layer this node belongs to (used by JS layer toggles)
        if USE_ENHANCED:
            parsed = parser.parse(node)
            layer_label = get_position_display_name(parsed['position']) if parsed.get('position') else f"Z {z:.1f}"
            node_layer_keys.append(layer_label)
        else:
            node_layer_keys.append(f"Z {z:.1f}")
        
        if USE_ENHANCED:
            # 'parsed' was already computed above for layer tracking
